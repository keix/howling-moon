/*
 * hm_lifecycle.c - Worker Lifecycle Management
 *
...
static ngx_pool_t *hm_runtime_pool = NULL;

ngx_int_t hm_init_process(ngx_cycle_t *cycle) {
  /*
   * Create a dedicated pool for runtime.
   * This pool lives for the entire worker process lifetime.
   */
  hm_runtime_pool = ngx_create_pool(4096, cycle->log);
  if (hm_runtime_pool == NULL) {
  ...
  hm_global_runtime = hm_runtime_create_stub(hm_runtime_pool, cycle->log);
  if (hm_global_runtime == NULL) {
    ngx_log_error(NGX_LOG_EMERG, cycle->log, 0,
                  "howling-moon: failed to create runtime");
    ngx_destroy_pool(hm_runtime_pool);
    hm_runtime_pool = NULL;
    return NGX_ERROR;
  }
...
void hm_exit_process(ngx_cycle_t *cycle) {
  if (hm_global_runtime != NULL) {
