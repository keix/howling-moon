...
struct hm_runtime_ops {
   ...
   * Returns NGX_OK on success, NGX_ERROR on failure.
   */
  ngx_int_t (*execute)(hm_runtime *rt, ngx_http_request_t *r,
                       ngx_chain_t **out);
};
...
/* Future:
 * hm_runtime *hm_runtime_create_mq(ngx_pool_t *pool, ngx_log_t *log);
 */
